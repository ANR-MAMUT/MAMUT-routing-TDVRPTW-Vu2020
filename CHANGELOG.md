...
**Re-priced under the `td-fold/2` checker contract (mamut-routing-lib 0.12.0); no route changed.** mamut-routing-lib 0.12.0 replaces the TD checker's route fold (checker contract `td-fold/1` -> `td-fold/2`): waiting and service at a vertex are now applied exactly to the accumulated arrival times instead of being composed through a ratio interpolation, the departure window restricts the first arc without interpolation, and travel on slope-one pieces is computed by addition. Under `td-fold/1` a ready time could be off by an ulp, and on stepwise travel-time functions such an ulp could land past a step and read its upper branch. All 168 BKS were re-priced with `mamut-routing bks reprice-td`: 161 files were rewritten, 56 of them because the cost moved (by at most 4.4e-16 relative, float rounding only), the other 105 only to refresh `route_durations` / `route_departure_times` by ulps. Each BKS whose cost moved records `metadata.repriced` (previous cost, checker, contract, date). 53 optimality-stamped BKS moved (by at most 1.4e-12): their `proven_optimum` now equals the re-priced cost, and a `note` records that the proof was obtained under `td-fold/1`, that `dual_bound` is the prover's value under that arithmetic, and that re-certification under `td-fold/2` is pending.

## 2026-08-06

**165 of the family's 166 pre-withdrawal optimality certificates re-derived from scratch and re-stamped** under the four-solve agreement protocol (cold and warm starts crossed with the two labeling modes, an audited exact-pricing phase in every run, zero checker-infeasible priced columns) on Grid'5000, on the repaired kayros 1.5.1 build. The campaign was motivated by the 2026-08-05 withdrawal of one certificate in the Vu2020 TDVRPTW family and the subsequent finding that the certifying builds carried since-repaired pricing defects (documented in the kayros 1.5.1 release notes and CHANGELOG). Every re-issued stamp certifies bit-exactly the previously stored value; no solution data changed, and the stamps' provenance now cites this campaign and build. Vu-A2-pB-d98-w100 (n=59, certificate withdrawn 2026-08-05) stays a plain best-known solution: the campaign again refused to certify it, with three solves proving 1846.8070188618972 and one labeling mode still closing at the withdrawn 1847.2395980636322, which is consistent with the withdrawal.

## 2026-08-05

**Vu-A2-pB-d98-w100 (n=59): optimality certificate WITHDRAWN, best known improved 1847.239598063632 -> 1846.8070188618972 (-0.0234 %, 12 routes).** A checker-valid solution strictly below the certified value (1847.2127599998892, 11 routes, found independently by two heuristic solver configurations) falsified the 2026-07-10 certificate. The investigation reproduced the certified value bit-for-bit at the certifying commit and traced the failure to a pricing defect repaired one commit later, on 2026-07-11: at abscissas where a partial route's duration is set-valued (a departure-choice set), the pricing read an arbitrary representative instead of the minimum, so improving columns were silently discarded, and the same incomplete pricing produced the matching dual bound. The defect is deterministic and identical across the protocol's four runs, so four-way agreement could not catch it on this instance; the certifying build, warm-started from the falsifying solution, itself proves the lower value. Re-solves on repaired builds prove 1846.8070188618972, validated bit-exactly by the canonical checker (an 11-route representation of the same optimum exists at 1846.8070188618974). The optimality claim is withdrawn and this entry is now a plain best-known solution. The family's remaining certificates are undergoing a full re-verification campaign on a repaired build, and optimality stamps will only be re-issued by that campaign.

## 2026-07-18

**Vu-A1-pA-d80-w180 (n=99) independently re-confirmed; no data changes.** The last main-wave refusal of this family (refused 2026-07-10 for checker-infeasible pricing in three of its four runs, then certified by the 2026-07-13 long-limit continuation) was re-run on the kayros 1.1.0 release candidate (commit f8c788c) under the full four-run protocol (time limit 7200 s, Grid'5000 reservation 6781269): all four solves reached Optimum at the stored proven optimum bit-exactly (maximum disagreement 0.0), each with an audited exact-pricing phase and zero checker-infeasible priced columns. The 2026-07-13 certificate stands re-validated on the current production build.
